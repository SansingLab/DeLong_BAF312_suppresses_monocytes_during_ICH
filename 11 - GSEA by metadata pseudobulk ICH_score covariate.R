...
mywd <- paste0("[path]/", myTime(), ' by metadata ICH_score covariate')
dir.create(mywd)
setwd(mywd)
  ...
  if (nrow(myData) > 0) { # Make sure there's data in the dataframe
    ...
    ggplot(myData, aes(x = pathway, y = NES, fill = timepoint, color = timepoint)) + 
      ...
      labs(y = 'NES', x = 'GSEA Hallmark Pathways',
           ...
           title = paste0('GSEA\n', myCompType, "\n", cellType, " - Top pathways Pseudobulk\nICH_score covariate removed\n")) +
      scale_y_continuous(limits = c(ifelse(min(myData$NES) < 0, min(myData$NES)*1.5, 0), ifelse(max(myData$NES) > 0, max(myData$NES)*1.5, 0)))
    ggsave(paste0("Barchart GSEA by metadata pseudobulk ICH_score covariate ", myCompType, " ", myComp, " ", cellType, " " , myTime(),  ".pdf"), 
           width = (3.3 + .35 * length(myPathways)), height = (4 + 0.06 * longestCPLabel), device = cairo_pdf) 
  }
}

# Guidance from https://bioinformaticsbreakdown.com/how-to-gsea/ (Brian Gudenas)
...
GSEA = function(gene_list, GMT_file, pval, myGeneset, myComparison, myTimepoint, myCluster) {
  ...
  message(paste("Number of NES > 0 = ", sum(fgRes$NES>0)))
  message(paste("Number of NES < 0 = ", sum(fgRes$NES<0)))
    ...
    g10= 
      ggplot(filtRes10, aes(NES, reorder(pathway, NES))) +
        ...
        labs(y = paste('GSEA', myGeneset), x = 'NES',
             ...
             title = paste0("GSEA Pseudobulk NoNPerm top 10\n", myCluster, " ", myTimepoint, "\n", myComparison, "\n", myGeneset, "\nICH_score covariate removed\n ")) +
        xlim(xmin - 0.2 * myRange, xmax + 0.2 * myRange)
      
    g10
    ggsave(paste0("Dotplot GSEA Pseudobulk ICH_score covariate NoNPerm top 10 ", myGeneset, " ", myComparison, " " , myCluster, " ", myTimepoint, " ", myTime(),  ".pdf"), 
           height = (2 + .25 * nrow(filtRes10)), width = (3.2 + 0.11 * longestLabel), device = cairo_pdf)
  }
  ...
  if (nrow(filtRes20) > 0) {
    ...
    g20= 
      ...
      labs(y = paste('GSEA', myGeneset), x = 'NES',
           size = '-log(FDR q-val)', color = 'Direction',
           title = paste0("GSEA Pseudobulk NoNPerm top 20\n", myCluster, " ", myTimepoint, "\n", myComparison, "\n", myGeneset, "\nICH_score covariate removed\n ")) +
      xlim(xmin - 0.2 * myRange, xmax + 0.2 * myRange)
    
    g20
    ggsave(paste0("Dotplot GSEA Pseudobulk ICH_score covariate NoNPerm top 20 ", myGeneset, " ", myComparison, " " , myCluster, " ", myTimepoint, " ", myTime(),  ".pdf"), 
           height = (2 + .25 * nrow(filtRes20)), width = (3.2 + 0.11 * longestLabel), device = cairo_pdf)
  }
...
#Load ranked DEG list
# ICH score covariate included, DEGs calculated with EdgeR
# DEGs calculated in script "08 - DEGs by metadata pseudobulk ICH score covariate.R"
load("[path]/EdgeR_Metadata_DEG_Lists_covariate_ICH_score.RData") 

GSEA_res_Metadata_Hallmark <- list()
for (i in 1:length(metadataEdgeRDEGTStatRanksVector)) {
  gene_list <- metadataEdgeRDEGTStatRanksVector[[i]]
  GMT_file = "[path]/msigdb_v2023.2.Hs_GMTs/symbols/h.all.v2023.2.Hs.symbols.gmt"
  
  geneset <- "MSigDB Hallmark Geneset"
  dataset <- str_split(names(metadataEdgeRDEGTStatRanksVector)[i], pattern = " ")[[1]]
  comparison <- dataset[1]
  timepoint <- str_c(dataset[(length(dataset) - 1):length(dataset)], collapse = " ")
  dataset <- dataset[c(-(1:4), -length(dataset), -(length(dataset) - 1))]
  cluster <- str_c(dataset, collapse = " ")
  
  res = GSEA(gene_list, GMT_file, pval = 0.05, geneset, comparison, timepoint, cluster)
  GSEA_res_Metadata_Hallmark[[names(metadataEdgeRDEGTStatRanksVector)[i]]] <- res
  gc()
}

#save(GSEA_res_Metadata_Hallmark, file = "GSEA_NoNPerm_res_Metadata_Hallmark.RData")
#load("[path]/GSEA_NoNPerm_res_Metadata_Hallmark.RData")
...
for (i in 1:length(GSEA_res_Metadata_Hallmark)) {
  df <- formatResults(GSEA_res_Metadata_Hallmark[[i]]$Results)
  write_delim(formatResults(GSEA_res_Metadata_Hallmark[[i]]$Results), file = paste0(names(GSEA_res_Metadata_Hallmark)[i], ".txt"), delim = '\t')
}


metadataComparisons <- unique(str_extract(names(GSEA_res_Metadata_Hallmark), '[:graph:]*\\s[:graph:]*\\s\\-\\s[:graph:]*'))

# Plot all timepoints
for (c in metadataComparisons) {
  for (pop in c('CD4+ T cells', 'CD8+ T cells', 'NK cells', 'B cells', 'Monocytes', 'CD16+ Monocytes')) {
    metadataComparisonType <- str_extract(c, '[:graph:]*')
    metadataComparison <- str_extract(c, '[:graph:]*\\s\\-\\s[:graph:]*')
    
    d1 <- GSEA_res_Metadata_Hallmark[[paste(c, pop, 'Day 1')]]$Results
    d3 <- GSEA_res_Metadata_Hallmark[[paste(c, pop, 'Day 3')]]$Results
    d7 <- GSEA_res_Metadata_Hallmark[[paste(c, pop, 'Day 7')]]$Results
    ...
    graphBarChartHorizontal(barchartData, pop, metadataComparisonType, metadataComparison)
  }
}
...
# Filter on day 1 values
nPathways <- nPathways[grep('Day8|Day15|Day30|Day90', nPathways$Metadata, invert = TRUE),]
# Plot
nPathways$Population <- factor(nPathways$Population, levels = c('Monocytes', 'CD16+ Monocytes', 'NK cells', 'CD4+ T cells', 'CD8+ T cells', 'B cells'))
ggplot(nPathways, aes(x = Population, y = NumPathways, fill = Timepoint)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), width = 0.7) +
  theme_bw() +
  facet_wrap(~Metadata, nrow = 3) +
  theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust = 1))
ggsave(paste0('Number GSEA Hallmark pathways associated with different Day 1 clinical characteristics ', myTime(), '.pdf'),
       height = 6, width = 8)


# Perform GSEA on Askenase 2021 datasets
# Load Askenase datasets
metadataTStatRanksAskenase <- list()
metadataTStatRanksAskenase[['Poor_vs_Good_Outcome_HF']] <- read.delim(file = '[path]/Askenase 2021 t statistic rnk files/Askenase 2021 - Poor vs Good Outcome DEGs HF.txt', header = FALSE, sep = "\t")
metadataTStatRanksAskenase[['Poor_vs_Good_Outcome_Blood']] <- read.delim(file = '[path]/Askenase 2021 t statistic rnk files/Askenase 2021 - Poor vs Good Outcome DEGs Blood.txt', header = FALSE, sep = "\t")

GSEA_res_Askenase_Metadata_Hallmark <- list()
for (i in 1:length(metadataTStatRanksAskenase)) {
  gene_list <- metadataTStatRanksAskenase[[i]]
  gene_list <- setNames(gene_list$V2, gene_list$V1)
  GMT_file = "[path]/msigdb_v2023.2.Hs_GMTs/symbols/h.all.v2023.2.Hs.symbols.gmt"
  
  geneset <- "MSigDB Hallmark Geneset"
  dataset <- "Askenase2021"
  comparison <- names(metadataTStatRanksAskenase)[i]
  timepoint <- "5-7d"
  
  cluster <- "Monos_Macs"
  
  res = GSEA(gene_list, GMT_file, pval = 0.05, geneset, comparison, timepoint, cluster)
  GSEA_res_Askenase_Metadata_Hallmark[[names(metadataTStatRanksAskenase)[i]]] <- res
  gc()
}
save(GSEA_res_Askenase_Metadata_Hallmark, file = "GSEA_NoNPerm_res_Askenase_Metadata_Hallmark.RData")
