...
mywd <- paste0("[path]/", myTime(), " Placebo by time equalNum down to 800 cells")
dir.create(mywd)
setwd(mywd)

# Load downsampled data from "06 - Equalize_cell_number_down_to_800_cells.R"
...
for (clusterArg in unique(s1p.combined.equalNum.Placebo$lineageClusterIdents)) {
  ...
  FindDEGsbyTime <- function(data, myClust) {
    ...
    # DEGs D3vD1
    DEGs_D3vD1 <- FindMarkers(myCluster, ident.1 = "Day 3", ident.2 = "Day 1", logfc.threshold = 0, min.pct = 0.1)
    write.table(DEGs_D3vD1, file = paste0("DEGs Placebo-treated equalNum down to 800 cells ", myClust, " Day 3 vs Day 1.txt"), 
                sep = "\t", col.names = NA)
    
    # DEGs D7vD1
    DEGs_D7vD1 <- FindMarkers(myCluster, ident.1 = "Day 7", ident.2 = "Day 1", logfc.threshold = 0, min.pct = 0.1)
    write.table(DEGs_D7vD1, file = paste0("DEGs Placebo-treated equalNum down to 800 cells ", myClust, " Day 7 vs Day 1.txt"), 
                sep = "\t", col.names = NA)
    
    # DEGs D7vD3
    DEGs_D7vD3 <- FindMarkers(myCluster, ident.1 = "Day 7", ident.2 = "Day 3", logfc.threshold = 0, min.pct = 0.1)
    write.table(DEGs_D7vD3, file = paste0("DEGs Placebo-treated equalNum down to 800 cells ", myClust, " Day 7 vs Day 3.txt"), 
                sep = "\t", col.names = NA)
  }
