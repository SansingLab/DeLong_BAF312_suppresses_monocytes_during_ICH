...
mywd <- paste0("[path]/", myTime(), ' by metadata equalNum down to 800 cells')
dir.create(mywd)
setwd(mywd)
...
FindDEGsbyMetadata <- function(data, myClust, t, metadataName, myContrast1, myContrast2) {
  ...
  write.table(DEGs, file = paste0("DEGs by metadata ", metadataName, " ", myContrast1, " - ", myContrast2, " equalNum down to 800 cells ", myClust, " ", t, ".txt"), 
              sep = "\t", col.names = NA)
}
