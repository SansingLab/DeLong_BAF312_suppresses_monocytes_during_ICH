...
mywd <- paste0("[path]/", myTime(), " BAF312 by time equalNum down to 1600 cells")
dir.create(mywd)
setwd(mywd)
...
for (clusterArg in unique(s1p.combined.equalNum.BAF312$lineageClusterIdents)) {
  ...
  FindDEGsbyTime <- function(data, myClust) {
    ...
    # DEGs D3vD1
    DEGs_D3vD1 <- FindMarkers(myCluster, ident.1 = "Day 3", ident.2 = "Day 1", logfc.threshold = 0, min.pct = 0.1)
    write.table(DEGs_D3vD1, file = paste0("DEGs BAF312-treated equalNum down to 1600 cells ", myClust, " Day 3 vs Day 1.txt"), 
                sep = "\t", col.names = NA)
    
    # DEGs D7vD1
    DEGs_D7vD1 <- FindMarkers(myCluster, ident.1 = "Day 7", ident.2 = "Day 1", logfc.threshold = 0, min.pct = 0.1)
    write.table(DEGs_D7vD1, file = paste0("DEGs BAF312-treated equalNum down to 1600 cells ", myClust, " Day 7 vs Day 1.txt"), 
                sep = "\t", col.names = NA)
    
    # DEGs D7vD3
    DEGs_D7vD3 <- FindMarkers(myCluster, ident.1 = "Day 7", ident.2 = "Day 3", logfc.threshold = 0, min.pct = 0.1)
    write.table(DEGs_D7vD3, file = paste0("DEGs BAF312-treated equalNum down to 1600 cells ", myClust, " Day 7 vs Day 3.txt"), 
                sep = "\t", col.names = NA)
  }
