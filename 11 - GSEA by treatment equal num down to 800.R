...
mywd <- paste0("[path]/", myTime(), " scDEG by Treatment equal num down to 800 cells")
dir.create(mywd)
setwd(mywd)
...
graphBarChartHorizontal <- function(myData, cellType) {
  if (nrow(myData) > 0) { # Make sure there's data in the dataframe
    ...
    ggsave(paste0("Barchart Horizontal GSEA Hallmark scDEGs by treatment equalNum down to 800 cells min_pct=0_1 ", cellType, " " , myTime(),  ".pdf"), 
...
GSEA = function(gene_list, GMT_file, pval, myGeneset, myComparison, myTimepoint, myCluster) {
    ...
    g10= 
      ggplot(filtRes10, aes(NES, reorder(pathway, NES))) +
        ...
        labs(y = paste('GSEA', myGeneset), x = 'NES',
             ...
             title = paste0("GSEA NoNPerm top 10\nBAF312 vs Placebo - scDEGs\nscDEGs equalNum; down to 800 cells; min.pct=0.1\n", myCluster, " ", myTimepoint, "\n", myGeneset, "\n ")) +
        xlim(xmin - 0.2 * myRange, xmax + 0.2 * myRange)
      
    g10
    ggsave(paste0("Dotplot GSEA scDEGs NoNPerm top 10 by Treatment equalNum down to 800 cells min_pct=0_1 ", myGeneset, " ", myCluster, " ", myTimepoint, " ", myTime(),  ".pdf"), 
  ...
  if (nrow(filtRes20) > 0) {
    ...
    g20
    ggsave(paste0("Dotplot GSEA scDEGs NoNPerm top 20 BAF312 by Treatment equalNum down to 800 cells min_pct=0_1 ", myGeneset, " ", myCluster, " ", myTimepoint, " ", myTime(),  ".pdf"), 
...
scDEGListbyTreatment <- list()
clusters <- c('Monocytes', 'CD16+ Monocytes', 'CD4+ T cells', 'CD8+ T cells', 'NK cells', 'B cells')
timepoints <- c('Day 1', 'Day 3', 'Day 7')
for (c in clusters) {
  for (t in timepoints) {
    # Load DEGs calculated in script "08 - DEGs by treatment equal num down to 800.R"
    filename <- paste0('[path]/DEGs by treatment equalNum down to 800 cells ', c, ' ', t, '.txt')
    if (file.exists(filename)){
      df <- read_delim(filename, delim = '\t')
      df <- df[order(df$avg_log2FC, decreasing = TRUE),]
      vector <- df$avg_log2FC
      names(vector) <- df$...1
      scDEGListbyTreatment[[paste('Treatment', c, t)]] <- vector
    }
  }
}

# Calculate and graph bubble plots
GSEA_res_Treatment_Hallmark <- list()
for (i in 1:length(scDEGListbyTreatment)) {
  gene_list <- scDEGListbyTreatment[[i]]
  GMT_file = "[path]/msigdb_v2023.2.Hs_GMTs/symbols/h.all.v2023.2.Hs.symbols.gmt"
  
  geneset <- "MSigDB Hallmark Geneset"
  dataset <- str_split(names(scDEGListbyTreatment)[i], pattern = " ")[[1]]
  ...
  GSEA_res_Treatment_Hallmark[[names(scDEGListbyTreatment)[i]]] <- res
  gc()
}
save(GSEA_res_Treatment_Hallmark, file = "GSEA_scDEGs_equalNum_down_to_800_cells_Treatment_Hallmark.RData")
...
for (pop in c('CD4+ T cells', 'CD8+ T cells', 'NK cells', 'B cells', 'Monocytes', 'CD16+ Monocytes')) {
  d1 <- GSEA_res_Treatment_Hallmark[[paste('Treatment', pop, 'Day 1')]]$Results
  d3 <- GSEA_res_Treatment_Hallmark[[paste('Treatment', pop, 'Day 3')]]$Results
  d7 <- GSEA_res_Treatment_Hallmark[[paste('Treatment', pop, 'Day 7')]]$Results
