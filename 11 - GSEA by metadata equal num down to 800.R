...
mywd <- paste0("[path]", myTime(), " scDEG by Metadata equal num down to 800 cells")
...
graphBarChartHorizontal <- function(myData, cellType, myCompType, myComp) {
  if (nrow(myData) > 0) { # Make sure there's data in the dataframe
    ...
    ggsave(paste0("Barchart GSEA scDEGs by ", myCompType, " ", myComp, " equalNum down to 800 cells ", cellType, " " , myTime(),  ".pdf"), 
...
GSEA = function(gene_list, GMT_file, pval, myGeneset, myComparison, myTimepoint, myCluster) {
  ...
  if (nrow(filtRes10) > 0) {
    ...
    g10
    ggsave(paste0("Dotplot GSEA top 10 by metadata down to 800 cells ", myComparison, " ", myGeneset, " ", myCluster, " ", myTimepoint, " ", myTime(),  ".pdf"), 
  ...
  if (nrow(filtRes20) > 0) {
    ...
    g20
    ggsave(paste0("Dotplot GSEA top 20 by metadata down to 800 cells ", myComparison, " ", myGeneset, " ", myCluster, " ", myTimepoint, " ", myTime(),  ".pdf"), 
...
# Path for DEGs calculated in script "08 - DEGs by metadata equal num down to 800.R"
DEG_dir <- '[path]/ by metadata equalNum down to 800 cells/'
...
for (cType in compType) {
  for (comp in comparison) {
    for (c in clusters) {
      for (t in timepoints) {
        filename <- paste0(DEG_dir, filename_start, cType, ' ', comp, ' equalNum down to 800 cells ', c, ' ', t, '.txt')
...
save(GSEA_res_Metadata_Hallmark, file = "GSEA_scDEGs_equalNum_down_to_800_Metadata_Hallmark.RData")

# Load previously run GSEA results
load("[path]/GSEA_scDEGs_equalNum_down_to_800_Metadata_Hallmark.RData")
