...
mywd <- paste0("[path]", myTime(), " for pseudobulk GSEA with ICH_score covariate by treatment no p-value")
dir.create(mywd)
setwd(mywd)
...
loadData <- function(cluster) {
  ...
  file_list <- file_list[grep("^Limma DEGs by treatment Day ", file_list)] # Filter on comparisons by treatment
  ...
  if (length(file_list_lineage) > 0) { # Make sure there is at least one file for this lineage
    ...
    for (t in timepoints) {
      ...
      if (length(file_list_lineage[grep(t, file_list_lineage)]) > 0) { # Make sure there's a file for this timepoint
        ...
        temp_data <- temp_data[c(1, 2, 4, 5, 6)] # Retain only gene, avg_log2FC, F statistic, p_val, p_val_adj
        colnames(temp_data) <- c('Gene', 'avg_log2FC', 'Fstat', 'p_val', 'p_val_adj')
    ...
    # Add -log(p-value)
    df$log_p_val <- ifelse(df$p_val == 0, 10^-300, df$p_val)
    df$log_p_val <- -log10(df$log_p_val)
    
    return(df)
  }
}

DotPlotSelectedGenesTreatment <- function(cluster, myFeatures) {
  ...
  ggplot(myData, aes(x = timepoint, y = Gene, 
                     #size = log_p_val, 
                     color = avg_log2FC)) + 
    ...
    labs(y = 'Gene', x = 'Timepoint',
         #size = 'F-statistic', 
         size = '-log(raw p-value)',
         color = 'avg_log2FC',
         title = paste(cluster, "- select genes"))
  ggsave(paste0("Dotplot DEGs by treatment ", cluster, " pseudobulk ICH_score covariate select genes by p-value ", myTime(),  ".pdf"), 
         height = (1.3 + .25 * length(myFeatures)), width = (3.5 + 0.055 * longestGeneLabel), device = cairo_pdf)
}

DEGdir <- "[path]/pseudobulk DEGs by treatment ICH_score covariate"

# Monocytes
df <- loadData('Monocytes')

# Load downsampled data from "07 - Combine downsampled objects.R"
load("[path]/s1p.downsampled.RData")

s1p.sub <- subset(s1p.downsampled, lineageClusterIdents == 'Monocytes') 
Idents(s1p.sub) <- s1p.sub$lineageClusterIdents

cluster <- 'Monocytes'

# Aided by Leading Edge genes from pseudobulk ICH_score covariate
myFeatures <- rev(c(
  'TNFAIP3',
  'NFKBIA',
  'FOS',
