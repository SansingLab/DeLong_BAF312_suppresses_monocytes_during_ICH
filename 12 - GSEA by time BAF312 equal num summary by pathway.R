...
mywd <- paste0("[path]/", myTime(), " scDEG by time BAF312 equal num summary by pathway")
dir.create(mywd)
setwd(mywd)
...
load("[path]/GSEA_scDEGs_equalNum_down_to_1600_cells_Time_BAF312_Hallmark_seed.RData")
...
plotSummary <- function(p) {
  ...
  ggsave(paste0("Dotplot GSEA scDEGs NoNPerm BAF312 by Time equalNum summary min_pct=0_1 ", myPathway, " ", myTime(),  ".pdf"), 
