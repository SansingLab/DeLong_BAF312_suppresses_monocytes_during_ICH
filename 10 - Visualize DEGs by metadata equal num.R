...
loadData <- function(cluster, metadataName, myContrast) {
  ...
  file_list <- file_list[grep(metadataName, file_list)] # Filter on type of comparison
  file_list <- file_list[grep(myContrast, file_list)] # Filter on actual comparison, just to make sure the comparison is correct
  file_list_lineage <- file_list[grep(cluster, file_list)]
  file_list_metadataName <- file_list_lineage[grep(metadataName, file_list_lineage)]
...
DotPlotSelectedGenesMetadata <- function(cluster, myFeatures, metadataName, myContrast) {
  ...
  ggsave(paste0("Dotplot DEGs by ", metadataName, ' ', myContrast, ' ', cluster, " scDEGs select genes ", myTime(),  ".pdf"), 
         height = (1.3 + .25 * length(myFeatures)), width = (4.5 + 0.055 * longestGeneLabel), device = cairo_pdf)
}
...
# Load DEGs from script 08 - DEGs by metadata equal num down to 1600.R
DEGdir <- "[path]/2024-10-03_142334 by metadata equalNum down to 1600 cells"
cluster <- 'Monocytes'
metadataName <- 'MRS_Day90_split_at_4'
contrast <- 'MRS_Day90_below_4 - MRS_Day90_above_or_equal_to_4'

df <- loadData(cluster, metadataName, contrast)
...
DotPlotSelectedGenesMetadata(cluster, myFeatures, metadataName, contrast)
