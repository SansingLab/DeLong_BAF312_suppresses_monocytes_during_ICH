...
mywd <- paste0("[path]/", myTime(), " equalized cell number down to 1600 cells setseed")
dir.create(mywd)
setwd(mywd)
  ...
  for (c in unique(x$lineageClusterIdents)) {
    ...
    for (p in unique(s1p.clust$patient_time_lineage)) {
      ...
      if (ncells < 1600) {
        ...
        set.seed(2222)
        newNormIDs <- sample(myCols, size = 1600, replace = FALSE)
        normIDs <- c(normIDs, newNormIDs)
        print(paste('ncells for', p, 'greater than 1600'))
      }
    }
  }
...
# Equalize the number of cells in each cluster for each sample down to 1600 cells
...
save(s1p.combined.equalNum, s1p.combined.equalNum.placebo, s1p.combined.equalNum.BAF312, file = 's1p.combined_equalNum_down_to_1600_cells.RData')
