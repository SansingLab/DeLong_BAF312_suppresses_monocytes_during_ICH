...
mywd <- paste0("[path]/", myTime(), ' by treatment equalNum down to 800 cells')
dir.create(mywd)
setwd(mywd)
...
for (clusterArg in unique(s1p.combined.equalNum$lineageClusterIdents)) {
  ...
  FindDEGsbyTreatment <- function(data, myClust, t) {
  	...
  	write.table(DEGs, file = paste0("DEGs by treatment equalNum down to 800 cells ", myClust, " ", t, ".txt"), 
  	            sep = "\t", col.names = NA)
  }
