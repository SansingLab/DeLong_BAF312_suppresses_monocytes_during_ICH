...
mywd <- paste0("[path]/", myTime(), " scDEG by time BAF312 equal num down to 800 cells")
dir.create(mywd)
setwd(mywd)
...
graphBarChartHorizontal <- function(myData, cellType) {
  if (nrow(myData) > 0) { # Make sure there's data in the dataframe
    ...
    ggsave(paste0("Barchart Horizontal GSEA Hallmark scDEGs by time BAF312 equalNum down to 800 cells min_pct=0_1 ", cellType, " " , myTime(),  ".pdf"), 
  ...
  if (nrow(myData) > 0) { # Make sure there's data in the dataframe
    ...
    ggplot(myData, aes(x = pathway, y = NES, fill = timepoint, color = timepoint)) + 
      ...
      labs(y = 'NES', x = 'GSEA Hallmark Pathways',
           ...
           title = paste(cellType, "- Top pathways (scDEGs)\nscDEGs equalNum\nDown to 800 cells\nmin.pct=0.1\n")) +
      scale_y_continuous(limits = c(ifelse(min(myData$NES) < 0, min(myData$NES)*1.5, 0), ifelse(max(myData$NES) > 0, max(myData$NES)*1.5, 0)))
    ggsave(paste0("Barchart Horizontal GSEA Hallmark scDEGs by time BAF312 D3vD1 and D7vD1 equalNum down to 800 cells min_pct=0_1 ", cellType, " " , myTime(),  ".pdf"), 
...
GSEA = function(gene_list, GMT_file, pval, myGeneset, myComparison, myTimepoint, myCluster) {
    ...
    g10= 
      ggplot(filtRes10, aes(NES, reorder(pathway, NES))) +
        ...
        labs(y = paste('GSEA', myGeneset), x = 'NES',
             ...
             title = paste0("GSEA NoNPerm top 10\nBAF312 by Time - scDEGs\nscDEGs equalNum; down to 800 cells; min.pct=0.1\n", myCluster, " ", myTimepoint, "\n", myGeneset, "\n ")) +
        xlim(xmin - 0.2 * myRange, xmax + 0.2 * myRange)
      
    g10
    ggsave(paste0("Dotplot GSEA scDEGs NoNPerm top 10 BAF312 by Time equalNum down to 800 cells min_pct=0_1 ", myGeneset, " ", myCluster, " ", myTimepoint, " ", myTime(),  ".pdf"), 
  ...
  if (nrow(filtRes20) > 0) {
    ...
    g20
    ggsave(paste0("Dotplot GSEA scDEGs NoNPerm top 20 BAF312 by Time equalNum down to 800 cells min_pct=0_1 ", myGeneset, " ", myCluster, " ", myTimepoint, " ", myTime(),  ".pdf"), 
...
for (c in clusters) {
  for (t in timepoints) {
    # Load DEGs calculated in script "08 - DEGs by time BAF312-treated equal num down to 800.R"
    filename <- paste0('[path]/DEGs BAF312-treated equalNum down to 800 cells ', c, ' ', t, '.txt')
    if (file.exists(filename)){
      ...
      scDEGListbyTimeBAF312[[paste('time', c, t)]] <- vector
    }
  }
}
...
for (i in 1:length(scDEGListbyTimeBAF312)) {
  gene_list <- scDEGListbyTimeBAF312[[i]]
  GMT_file = "[path]/msigdb_v2023.2.Hs_GMTs/symbols/h.all.v2023.2.Hs.symbols.gmt"
  
  geneset <- "MSigDB Hallmark Geneset"
  dataset <- str_split(names(scDEGListbyTimeBAF312)[i], pattern = " ")[[1]]
  ...
  GSEA_res_Time_Hallmark[[names(scDEGListbyTimeBAF312)[i]]] <- res
  gc()
}
save(GSEA_res_Time_Hallmark, file = "GSEA_scDEGs_equalNum_down_to_800_cells_Time_BAF312_Hallmark.RData")
