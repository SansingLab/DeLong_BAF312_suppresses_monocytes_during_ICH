...
mywd <- paste0("[path]/", myTime(), " scDEG by Treatment equal num down to 1600 cells")
dir.create(mywd)
setwd(mywd)
...
graphBarChartHorizontal <- function(myData, cellType) {
  if (nrow(myData) > 0) { # Make sure there's data in the dataframe
    ...
    ggsave(paste0("Barchart Horizontal GSEA Hallmark scDEGs by treatment equalNum down to 1600 cells min_pct=0_1 ", cellType, " " , myTime(),  ".pdf"), 
...
GSEA = function(gene_list, GMT_file, pval, myGeneset, myComparison, myTimepoint, myCluster) {
  ...
  if (nrow(filtRes10) > 0) {
    ...
    g10
    ggsave(paste0("Dotplot GSEA scDEGs NoNPerm top 10 by Treatment equalNum down to 1600 cells min_pct=0_1 ", myGeneset, " ", myCluster, " ", myTimepoint, " ", myTime(),  ".pdf"), 
  ...
  if (nrow(filtRes20) > 0) {
    ...
    g20
    ggsave(paste0("Dotplot GSEA scDEGs NoNPerm top 20 BAF312 by Treatment equalNum down to 1600 cells min_pct=0_1 ", myGeneset, " ", myCluster, " ", myTimepoint, " ", myTime(),  ".pdf"), 
...
for (c in clusters) {
  for (t in timepoints) {
    # Load DEGs calculated in script "08 - DEGs by treatment equal num down to 1600.R"
    filename <- paste0('[path]/DEGs by treatment equalNum down to 1600 cells ', c, ' ', t, '.txt')
...
save(GSEA_res_Treatment_Hallmark, file = "GSEA_scDEGs_equalNum_down_to_1600_cells_Treatment_Hallmark.RData")
