...
mywd <- paste0("[path]/", myTime(), " scDEG by time Placebo equal num down to 1600 cells")
dir.create(mywd)
setwd(mywd)
...
graphBarChartHorizontal <- function(myData, cellType) {
  if (nrow(myData) > 0) { # Make sure there's data in the dataframe
    ...
    ggsave(paste0("Barchart Horizontal GSEA Hallmark scDEGs by time Placebo equalNum down to 1600 cells min_pct=0_1 ", cellType, " " , myTime(),  ".pdf"), 
  ...
  if (nrow(myData) > 0) { # Make sure there's data in the dataframe
    ...
    ggplot(myData, aes(x = pathway, y = NES, fill = timepoint, color = timepoint)) + 
      ...
      labs(y = 'NES', x = 'GSEA Hallmark Pathways',
           ...
           title = paste(cellType, "- Top pathways (scDEGs)\nscDEGs equalNum\nDown to 1600 cells\nmin.pct=0.1\n")) +
      scale_y_continuous(limits = c(ifelse(min(myData$NES) < 0, min(myData$NES)*1.5, 0), ifelse(max(myData$NES) > 0, max(myData$NES)*1.5, 0)))
    ggsave(paste0("Barchart Horizontal GSEA Hallmark scDEGs by time Placebo D3vD1 and D7vD1 equalNum down to 1600 cells min_pct=0_1 ", cellType, " " , myTime(),  ".pdf"), 
...
GSEA = function(gene_list, GMT_file, pval, myGeneset, myComparison, myTimepoint, myCluster) {
  ...
  if (nrow(filtRes10) > 0) {
    ...
    g10
    ggsave(paste0("Dotplot GSEA scDEGs NoNPerm top 10 Placebo by Time equalNum down to 1600 cells min_pct=0_1 ", myGeneset, " ", myCluster, " ", myTimepoint, " ", myTime(),  ".pdf"), 
  ...
  if (nrow(filtRes20) > 0) {
    ...
    g20
    ggsave(paste0("Dotplot GSEA scDEGs NoNPerm top 20 Placebo by Time equalNum down to 1600 cells min_pct=0_1 ", myGeneset, " ", myCluster, " ", myTimepoint, " ", myTime(),  ".pdf"), 
...
for (c in clusters) {
  for (t in timepoints) {
    ...
    filename <- paste0('[path]/DEGs Placebo-treated equalNum down to 1600 cells ', c, ' ', t, '.txt')
...
save(GSEA_res_Time_Hallmark, file = "GSEA_scDEGs_equalNum_down_to_1600_cells_Time_Placebo_Hallmark.RData")
