...
mywd <- paste0("[path]/", myTime(), " equalized cell number down to 800 cells setseed")
dir.create(mywd)
setwd(mywd)
  ...
  for (c in unique(x$lineageClusterIdents)) {
    ...
    for (p in unique(s1p.clust$patient_time_lineage)) {
      ...
      if (ncells < 800) {
        ...
        set.seed(2222)
        newNormIDs <- sample(myCols, size = 800, replace = FALSE)
        normIDs <- c(normIDs, newNormIDs)
        print(paste('ncells for', p, 'greater than 800'))
      }
    }
  }
...
# Equalize the number of cells in each cluster for each sample down to 800 cells
...
save(s1p.combined.equalNum, s1p.combined.equalNum.placebo, s1p.combined.equalNum.BAF312, file = 's1p.combined_equalNum_down_to_800_cells.RData')
